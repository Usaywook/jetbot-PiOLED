...
| `TEMP_ALERT_REPEAT_S` | 임계 이상 지속 시 재알림 간격 | 0 또는 미설정 → 단발 알림 (재알림 OFF). 양수면 그 간격마다 `[ALERT-REPEAT]` 발송 |

값이 잘못되었거나 비어있으면 코드 내 기본값(80°C / 30s / 60s / 재알림 off) 적용 + stderr 로 경고. Webhook 호출 실패는 stderr 로만 로그하고 루프는 계속 돕니다.
