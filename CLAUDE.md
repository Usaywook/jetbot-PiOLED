...
- `pioled/stats.py` — 메인 루프. 4 FPS로 4줄(IP / GPU 막대 / Mem / Disk)을 그립니다. 수정 대상은 보통 이 파일 하나.
...
| 네트워크 인터페이스 | `eth0` 고정 | `get_active_interface()` 가 `lo/docker0/l4tbr0/veth*` 제외 후 첫 `up` 인터페이스 선택 |
| 폰트 크기 | 기본 | `ImageFont.load_default(size=8)` — 32px 높이에 4줄(0/8/16/24) 정확히 들어가도록 |

`stats.py` 의 그리기 좌표는 8px 폰트 + 32px 디스플레이를 가정합니다. 폰트 크기를 바꾸면 4줄이 잘립니다.
